...
public class CharacterControllerD : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        foreach (ContactPoint[] contacts in contactPoints.Values)
            for (int i = 0; i < contacts.Length; i++)
                if (contacts[i].point.y <= rig.position.y - bottomCapsuleSphereOrigin && Physics.Raycast(contacts[i].point + Vector3.up, Vector3.down, out hit, 1.1f, ~0) && Vector3.Angle(hit.normal, Vector3.up) <= MaximumSlope)
                {
                    grounded = true;
        ...
        if (grounded && doJump != 3)
        {
            ...
            float difference = length - rig.velocity.magnitude;
            ...
            else
            {
                ...
                movement = new Vector3((targetSpeed.x - rig.velocity.x) * difference * acceleration, (targetSpeed.y - rig.velocity.y) * difference * acceleration, (targetSpeed.z - rig.velocity.z) * difference * acceleration);
            }
 
            if (doJump == 1)
            {
                // jump button was pressed, do jump      
                movement.y = JumpSpeed - rig.velocity.y;
                doJump = 2;
            }
            ...
            rig.AddForce(new Vector3(movement.x, movement.y, movement.z), ForceMode.VelocityChange);
        ...
        else
        {
            ...
            if (groundedLastFrame && doJump != 3 && !falling)
            {
                ...
                if (Physics.Raycast(transform.position, Vector3.down, out hit, fudgeCheck + (rig.velocity.magnitude * Time.deltaTime), ~0) && Vector3.Angle(hit.normal, Vector3.up) <= MaximumSlope)
                {
                    groundedLastFrame = true;
                    ...
                    {
                        rig.AddForce(hit.normal * -hit.distance, ForceMode.VelocityChange);
            ...
            if (!Mathf.Approximately(inputX + inputY, 0.0f))
            {
                ...
                float a = movement.x + rig.velocity.x;
                float b = movement.z + rig.velocity.z;
                ...
                if (length > 0.0f)
                {
                    if (length > MovementSpeed)
                    {
                        ...
                        length = 1.0f / Mathf.Sqrt(rig.velocity.x * rig.velocity.x + rig.velocity.z * rig.velocity.z);
                        Vector3 rigidbodyDirection = new Vector3(rig.velocity.x * length, 0.0f, rig.velocity.z * length);
                    ...
                    rig.AddForce(new Vector3(movement.x, 0.0f, movement.z), ForceMode.VelocityChange);
                }
            }
